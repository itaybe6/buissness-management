-- ============================================================================
-- 044: שכר איש תחזוקה לפי תקלה — מחיר עבודה + אישור מנהל
-- אחרי סיום תקלה איש התחזוקה מזין מחיר; לאחר אישור מנהל — נספר בשכר.
-- ============================================================================
...
comment on column public.faults.work_price is 'מחיר העבודה שאיש התחזוקה מזין לאחר סיום התקלה';
comment on column public.faults.pay_employee_id is 'איש התחזוקה שיקבל את התשלום';
comment on column public.faults.pay_approval_status is 'pending = ממתין לאישור מנהל; approved = נספר בשכר';
