...
-- ============================================================================
-- 17. כיבוי מודול = מחיקת הדאטה שלו (סופר אדמין)
--
--     כיבוי מודול אינו הסתרת תפריט: הוא מוחק את הנתונים של המודול לאותו עסק.
--     מקבילה ב-TypeScript: src/lib/featureData.ts. ההתאמה בין השניים נבדקת
--     ב-tests/super-admin/featurePurge.test.ts.
--
--     קבצים ב-Storage לא נמחקים כאן (מחיקה מ-storage.objects דרך SQL משאירה
--     את הקובץ ב-bucket) — הניקוי שלהם רץ מהקליינט דרך storage API.
-- ============================================================================
...
create table if not exists public.business_feature_purges (
  ...
  deleted      jsonb not null default '{}'::jsonb,   -- טבלה → מספר שורות שנמחקו
...
-- מפת בעלות: איזה טבלאות שייכות לאיזה מודול. כל טבלה שייכת למודול אחד בדיוק;
-- טבלאות משותפות (shift_templates, departments, profiles) הן הגדרות עסק ולא
-- מופיעות כאן. הסדר בתוך כל מודול הוא סדר מחיקה: ילדים לפני הורים.
...
-- סדר מחיקה גלובלי — כמה מודולים בלחיצה אחת חייבים להימחק בסדר שלא שובר FK.
...
-- דוח מקדים לחלון האישור: כמה שורות באמת ימחקו. read-only.
...
-- המחיקה עצמה. security definer עוקף RLS בכוונה (סופר אדמין לא שייך לעסק),
-- והבדיקה הראשונה היא is_super_admin().
...
-- כתיבת מצב המודולים + מחיקה בטרנזקציה אחת: אם המחיקה נכשלת, גם הדגלים
-- חוזרים לאחור, כדי שלא להישאר עם מודול כבוי ודאטה חצי-מחוקה.
