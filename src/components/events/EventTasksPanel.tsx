...
import { EmployeeMultiPicker, namesSummary } from "@/components/events/EmployeeMultiPicker";
import {
  useCreateTask,
  useDeleteTask,
...
export function EventTasksPanel({
  ...
  /** Staff who can actually be handed a task — active, and not platform admins. */
  ...
  const selectedNames = useMemo(
    () =>
      assignedIds
        .map((id) => users.find((u) => u.id === id)?.full_name || "ללא שם")
        .filter(Boolean),
    [assignedIds, users],
  );
  ...
  const submitLabel =
    assignMode === "employee" && assignedIds.length > 1
      ? `הוספה ל־${assignedIds.length} עובדים`
      : "הוספת משימה";
  ...
  async function handleAdd() {
    ...
    if (isDept ? !departmentId : assignedIds.length === 0) {
      setAddError(isDept ? "נא לבחור מחלקה" : "נא לבחור לפחות עובד אחד למשימה");
      return;
    }
    ...
    try {
      if (isDept) {
        // One shared row for the whole department — marking it done closes it for everyone.
        await createTask.mutateAsync({ ...base, department_id: departmentId, assigned_to: null });
      } else {
        // One row per employee — each sees their own copy on their home screen and can
        // complete it independently.
        const ids = await Promise.all(
          assignedIds.map((assignedTo) =>
            createTask.mutateAsync({ ...base, department_id: null, assigned_to: assignedTo }),
          ),
        );
        ids.forEach((id) => notifyTaskAssigned(id));
      }
      closeAddModal();
    } catch {
    ...
    <>
      ...
      {canCreate && (
        <Modal
          ...
          subtitle={`${event.title} · שיוך לעובדים או למחלקה`}
          icon="checklist"
          maxWidth={560}
          fullScreenMobile
          footer={
                ...
                <>
                  ...
                  {assignedIds.length > 1 && (
                    <p className="mt-2 text-[12px] font-semibold text-text-2">
                      תיווצר משימה נפרדת לכל אחד מ־{assignedIds.length} העובדים — כל אחד מסמן ביצוע בעצמו
                    </p>
                  )}
                </>
              ) : (
